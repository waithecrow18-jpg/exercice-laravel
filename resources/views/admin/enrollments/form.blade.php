    ...
    <div>
        ...
        <select name="user_id" class="w-full rounded-2xl border border-slate-200 px-4 py-3">
            @foreach ($users as $userItem)
                <option value="{{ $userItem->id }}" @selected(old('user_id', $enrollment->user_id ?? '') == $userItem->id)>{{ $userItem->name }}</option>
            @endforeach
        </select>
    </div>
    <div>
        ...
        <select name="training_session_id" class="w-full rounded-2xl border border-slate-200 px-4 py-3">
            @foreach ($sessions as $sessionItem)
                <option value="{{ $sessionItem->id }}" @selected(old('training_session_id', $enrollment->training_session_id ?? '') == $sessionItem->id)>{{ $sessionItem->training?->title_fr }} - {{ $sessionItem->starts_at?->format('Y-m-d') }}</option>
            @endforeach
        </select>
    </div>
    <div>
        ...
        <select name="status" class="w-full rounded-2xl border border-slate-200 px-4 py-3">
            @foreach ($statuses as $status)
                <option value="{{ $status->value }}" @selected(old('status', $enrollment->status->value ?? 'pending') === $status->value)>{{ $status->label() }}</option>
            @endforeach
        </select>
    </div>
