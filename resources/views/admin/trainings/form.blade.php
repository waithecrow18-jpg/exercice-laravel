<div class="grid gap-5 md:grid-cols-2">
    <div>
        ...
        <select name="category_id" class="w-full rounded-2xl border border-slate-200 px-4 py-3">
            @foreach ($categories as $category)
                <option value="{{ $category->id }}" @selected(old('category_id', $training->category_id ?? '') == $category->id)>{{ $category->name_fr }}</option>
            @endforeach
        </select>
    </div>
    <div>
        ...
        <select name="status" class="w-full rounded-2xl border border-slate-200 px-4 py-3">
            @foreach ($statuses as $status)
                <option value="{{ $status->value }}" @selected(old('status', $training->status->value ?? 'draft') === $status->value)>{{ $status->label() }}</option>
            @endforeach
        </select>
    </div>
    <div>
    ...
    <div>
        ...
        <input type="datetime-local" name="published_at" value="{{ old('published_at', isset($training) && $training->published_at ? $training->published_at->format('Y-m-d\TH:i') : '') }}" class="w-full rounded-2xl border border-slate-200 px-4 py-3">
    </div>
    <div class="md:col-span-2">
